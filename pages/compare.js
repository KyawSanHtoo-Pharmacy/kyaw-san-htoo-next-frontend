  ...
  return (
    <>
      ...
      <Compare.Section>
        <GlobalContainer>
          <Compare
            outstockMedicine={outstockMedicine}
            instockMedicine={instockMedicine}
          />
          <Accordion mb="6.25em">
            {productDetailsCompareData.map(
              ({ id, title, outstockBody, instockBody }) => (
                <Accordion.Item key={id}>
                  <Accordion.Title>{title}</Accordion.Title>
                  <Accordion.AnswerWrapper>
                    <Accordion.TwoColAnswer>
                      <Accordion.TwoColAnswerColumn>
                        <NoticePill availability={false}>
                          {outstockMedicine.product_name_mm}
                        </NoticePill>
                      ...
                      <Accordion.TwoColAnswerColumn>
                        <NoticePill availability={true}>
                          {instockMedicine.product_name_mm}
                        </NoticePill>
                        <Accordion.Answer m="0.5em 0 0 0">
                          {parse(instockBody)}
                        </Accordion.Answer>
            ...
            >
              <Link href={`/categories/all`} passHref>
                <Button.White style={{ marginRight: "1em" }}>
                  ဆေးမျိုးစုံပေ့ခ်ျကိုပြန်သွားမယ်
                </Button.White>
              </Link>
              <Link
                href={`/categories/${instockMedicine.categories[0].slug}/${instockMedicine.slug}`}
                passHref
              >
  ...
  return {
    props: {
      ...
      relatedMedicines: relatedMedicines.filter(
        (medicine) =>
          medicine.slug !== outstockMedicine[0].slug &&
          medicine.slug !== instockMedicine[0].slug
      ),
    },
  };
}
